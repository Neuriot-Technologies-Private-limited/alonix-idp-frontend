...
export const BillingPage: React.FC = () => {
  ...
  const pageShell = 'w-full max-w-6xl mx-auto';

  if (isLoading) {
    return (
      <div className={cn(pageShell, 'flex min-h-[60vh] items-center justify-center pb-16')}>
  ...
  if (error || !data) {
    return (
      <div className={cn(pageShell, 'flex min-h-[60vh] flex-col items-center justify-center gap-3 pb-16 text-center')}>
  ...
  return (
    <div className={cn(pageShell, 'space-y-6 pb-16 animate-in fade-in duration-500')}>
      ...
      <div
        className={cn(
          'relative overflow-hidden rounded-3xl border border-border/20 p-6 sm:p-8 shadow-2xl shadow-black/20',
          'bg-gradient-to-br from-primary/15 via-surface-highest/20 to-background'
        )}
      >
        <div className="pointer-events-none absolute -right-16 -top-16 h-48 w-48 rounded-full bg-primary/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-20 left-1/3 h-40 w-40 rounded-full bg-violet/10 blur-3xl" />
        <div className="relative flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex shrink-0 items-center gap-2 rounded-xl border border-border/20 bg-surface-highest/10 px-3 py-2">
            <span className={cn('text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full', statusColor(subscription.status))}>
              {subscription.status}
            </span>
            <span className="text-sm font-black text-foreground">{plan.displayName}</span>
          </div>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-0.5">{productPlanDescription(plan.description)}</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-12">
        ...
        {/* Plan limits */}
        <div className="xl:col-span-4 grid grid-cols-2 gap-3 xl:grid-cols-1">
          {[
            ...
            <div key={label} className="rounded-2xl border border-border/10 bg-surface-lowest p-4 text-center shadow-lg xl:flex xl:items-center xl:gap-4 xl:text-left">
              <div className="flex justify-center mb-2 xl:mb-0 xl:shrink-0">{icon}</div>
              <div>
